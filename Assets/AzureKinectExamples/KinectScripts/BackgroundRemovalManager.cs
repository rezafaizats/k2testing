    ...
    public class BackgroundRemovalManager : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (bBackgroundRemovalInited)
            {
                ...
                {
                    foregroundImage.texture = foregroundTex;
                    foregroundImage.rectTransform.localScale = kinectManager.GetColorImageScale(sensorIndex);
                    foregroundImage.color = new Color(1f, 1f, 1f, opaqueness);  // Color.white;
                }

                if (foregroundTex != null && foregroundTexture != null)
                {
                    Vector2 imageScale = sensorData.colorImageScale;
                    KinectInterop.TransformTexture(foregroundTex, foregroundTexture, 0, imageScale.x < 0f, imageScale.y < 0f, foregroundCamera == null);
                }
            }
        }
        ...
        private bool InitBackgroundRemoval(KinectInterop.SensorData sensorData)
        {
            if (sensorData != null && sensorData.sensorInterface != null && KinectInterop.IsDirectX11Available())
            {
                ...
                foregroundTex = KinectInterop.CreateRenderTexture(foregroundTex, textureRes.x, textureRes.y, RenderTextureFormat.ARGB32);
        ...
        private bool UpdateBackgroundRemoval(KinectInterop.SensorData sensorData)
        {
                ...
                lastColorBodyIndexBufferTime != sensorData.lastColorBodyIndexBufferTime || filterByGS != null))
            {
                ...
                if(!computeAlphaMaskOnly)
                {
                    ...
                    ApplySimpleFilter(alphaTexture, foregroundTex, foregroundMat, tempTextures);
                }
                else
                {
                    Graphics.CopyTexture(alphaTexture, foregroundTex);
                }
