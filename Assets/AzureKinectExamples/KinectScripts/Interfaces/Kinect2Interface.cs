...
namespace com.rfilkov.kinect
{
    ...
    public class Kinect2Interface : DepthSensorBase
    {
        ...
        public override void CalcBodySpecialJoints(ref KinectInterop.BodyData bodyData)
        {
            // clavicle right
            {
        ...
        public override void PollCoordTransformFrames(KinectInterop.SensorData sensorData)
        {
            if (lastDepthCoordFrameTime != rawDepthTimestamp)
            {
                ...
                {
                    if (depthCamColorCoordFrame.Length != (sensorData.depthImageWidth * sensorData.depthImageHeight))
                    {
                    ...
                    lock (depthCamColorFrameLock)
                    {
                        ...
                        //Debug.Log($"depthImageLen: {rawDepthImage.Length}, colorCoordFrameLen: {depthCamColorCoordFrame.Length}");
                ...
                // color2depth frame
                if (colorCamDepthCoordFrame != null && rawDepthImage != null)
                {
                    if (colorCamDepthCoordFrame.Length != (sensorData.colorImageWidth * sensorData.colorImageHeight))
                    {
                    ...
                    lock (colorCamDepthFrameLock)
                    {
                        ...
                        //Debug.Log($"depthImageLen: {rawDepthImage.Length}, depthCoordFrameLen: {colorCamDepthCoordFrame.Length}");
