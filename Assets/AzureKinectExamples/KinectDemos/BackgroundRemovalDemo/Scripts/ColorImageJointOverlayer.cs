        ...
        void Update()
        {
            if (kinectManager && kinectManager.IsInitialized())
            {
                ...
                if (!planeRectSet && planeObject)
                {
                    ...
                    planeRect.height = /**10f * */ Mathf.Abs(planeObject.localScale.y);
                    planeRect.x = planeObject.position.x - planeRect.width * 0.5f;
                    planeRect.y = planeObject.position.y - planeRect.height * 0.5f;
                }
                ...
                if (planeObject && kinectManager.IsJointTracked(userId, iJointIndex))
                {
                    ...
                    //Debug.Log($"posJoint: {posJoint:F2}");

                    if (overlayObject && posJoint != Vector3.zero)
                    {
