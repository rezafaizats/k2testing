        ...
        void Update()
        {
            ...
            if (kinectManager && kinectManager.IsInitialized())
            {
                ...
                if (kinectManager.IsUserDetected(playerIndex))
                {
                    ...
                    ulong bodyId = kinectManager.GetSensorBodyId(sensorIndex, userId);
                    int bodyIndex = kinectManager.GetSensorBodyIndex(sensorIndex, bodyId);

                    if (kinectManager.IsSensorJointTracked(sensorIndex, bodyIndex, iJointIndex))
                    {
                        Vector3 posJointRaw = kinectManager.GetSensorJointKinectPosition(sensorIndex, bodyIndex, iJointIndex, false);
